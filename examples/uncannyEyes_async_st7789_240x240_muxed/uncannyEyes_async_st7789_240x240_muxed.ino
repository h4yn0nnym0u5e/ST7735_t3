...
typedef struct {              // Struct is defined before including config.h --
  void  (*CSfn)(bool negate); // Chip select function
  int8_t  dc;                 // DC pin
  int8_t  rst;                // reset pin
  int8_t  wink;               // and wink button (or -1 if none) specified there,
  uint8_t rotation;           // also display rotation.
 ...
 * 2 displays, using only 2 Teensy outputs (we could 
 * address up to 8).
 */
...
#if defined(MUXED_CS) 
    eye[e].display = new displayType(eyeInfo[e].CSfn, eyeInfo[e].dc, -1);
    (*eyeInfo[e].CSfn)(HIGH);
#else
...
#if defined(MUXED_CS)
  eye[0].display->setCustomRotation(0x20, 240, 240, 0, 0);
#else
